<template>
  <div>
    <h2 class="text-2xl mb-1">{{ $t('reports.paymentCharts.tabs.byTypeEachMonth.header') }}</h2>

    <div class="flex flex-col justify-end"
         v-if="this.allYears.length > 0"
    >
      <div>
        <h2 class="text-lg text-center md:text-right pr-0 md:pr-6 mb-1">{{ $t('reports.paymentCharts.tabs.byTypeEachMonth.filter.filterByYear.header') }}</h2>
      </div>

      <div class="flex flex-col md:flex-row justify-end">
        <MediumButtonWithIcon :text="$t('reports.paymentCharts.tabs.byTypeEachMonth.filter.filterByYear.showAll.label')"
...
export default {
      ...
      return {
        ...
        options: {
          scales: {
            ...
            y: {
              grace: '10%', // adds offset from top, so if yMax=500, then chart will show 550/600 as max
              min: 0, // grace creates offset also below 0, thus cutting it off by hardcoding 0 point
              title: {
                display: true,
  ...
  async mounted(): Promise<void> {
    this.chartData = await this.$moduleCall.getAll(SymfonyReportsRoutes.PAYMENTS_TOTAL_PER_TYPE_BASE_URL)
    this.buildYears();
