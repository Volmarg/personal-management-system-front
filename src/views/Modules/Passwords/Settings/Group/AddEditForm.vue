  ...
  <div class="flex justify-center">
    <div class="mt-6 md:w-1/2 lg:w-1/3 w-full flex flex-col">
      ...
      <FormInput type="text"
                 v-model="form.name"
                 :is-required="true"
                 :label="$t('passwords.settings.groups.form.new.name.label')"
      />

      <MediumButtonWithIcon :text="$t('passwords.settings.groups.form.new.submitButton.label')"
...
export default {
  ...
  methods: {
     ...
     */
    async onSubmit(): void {
      let config = new BackendModuleCallConfig(SymfonyPasswordsRoutes.PASSWORDS_GROUPS_BASE_URL, this.id, BaseApiResponse, this.form);
      config.reload = false;
